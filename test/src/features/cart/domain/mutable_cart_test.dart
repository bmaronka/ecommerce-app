...
void main() {
  group(
    ...
    () {
      test(
        'empty cart - set item with quantity',
        () {
      ...
      test(
        'cart with same item - override quantity',
        () {
      ...
      test(
        'cart with different item - set item with quantity',
        () {
          final cart = const Cart()
  ...
  group(
    ...
    () {
      ...
      test(
        'empty cart - add same item twice',
        () {
          final cart = const Cart()
              .addItem(const Item(productId: '1', quantity: 1))
              .addItem(const Item(productId: '1', quantity: 1));
  ...
  group(
    ...
    () {
      ...
      test(
        'cart with one item - add two items of which one matching',
        () {
      ...
      test(
        'cart with one item - add two new items',
        () {
          final cart = const Cart().addItem(const Item(productId: '1', quantity: 1)).addItems([
            const Item(productId: '2', quantity: 1),
  ...
  group(
    ...
    () {
      ...
      test(
        'empty cart - remove matching item',
        () {
          final cart = const Cart().addItem(const Item(productId: '1', quantity: 1)).removeItemById('1');
          expect(cart.items, {});
        },
      );

      test(
        'empty cart - remove non-matching item',
        () {
          final cart = const Cart().addItem(const Item(productId: '2', quantity: 1)).removeItemById('1');
